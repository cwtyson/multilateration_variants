localize_fn <- function(prepared_file,
  ...
  rssi_dist_model_file = 'data/RSSI_nls_dist_model_2023.RDS'
  ...
  new_data = data.frame(rssi = seq(max(dets_p$rssi_gp_r), min(dets_p$rssi_gp_r), by = -1))
  ...
  if(log == TRUE){
    ...
    ## Model parameters
    a = -coef(rssi_dist_mdl)[2]
    S = exp(coef(rssi_dist_mdl)[3])
    K = coef(rssi_dist_mdl)[1]
    ...
    pred_df = map(.x = rev(sort(unique(mdl_data$rssi))),
      ...
      ## Read in model
      rssi_dist_log_mdl <- readRDS(rssi_dist_model_file) 
