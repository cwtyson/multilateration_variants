...
# import data
data_df <- read.csv("data/time_diff_matched_df.csv")
...
test_info <- df %>% dplyr::select(row_id, NodeId, TagId, maxRSS, sdRSS, n.det, elevation,
                                       trial_num=unique_trial_id, lon, lat, tag_type, test_local_time, start_time, end_time,
...
node_ids <- unique(df_model1$NodeId)
for (node_id in node_ids) {
  
  df_node <- df_model1 %>% filter(NodeId == node_id)
